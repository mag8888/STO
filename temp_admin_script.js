        ...
        function renderChatList(fullList) {
            ...
            if (currentFilter !== 'ALL') {
                list = list.filter(d => {
                    if (currentFilter === 'SCOUT') return d.source === 'SCOUT';
                    if (currentFilter === 'INBOUND') return d.source !== 'SCOUT'; // Assume anything not SCOUT is INBOUND/Direct
                    return true;
                });
            }
        ...
        async function selectChat(id) {
            ...
            if (d.user.status === 'LEAD') statusText += ' • 🔥 Lead';
            if (d.user.status === 'REJECTED') statusText += ' • 🚫 Rejected';
        ...
        function renderChatHeader(dialogue) {
            ...
            if (user.status === 'LEAD') statusText += ' • 🔥 Lead';
            if (user.status === 'REJECTED') statusText += ' • 🚫 Rejected';
            statusEl.textContent = statusText;
        ...
        async function archiveChat(id, skipConfirm = false) {
            ...
            try {
                ...
                if (dialogues.length > 0) {
                    ...
                    document.getElementById('messages-area').innerHTML = '<div style="padding:2rem; text-align:center; color:var(--text-secondary);">All caught up! 🎉</div>';
        ...
        function renderMessages(messages) {
            ...
            if (lastMsg && lastMsg.sender === 'USER' && lastMsg.status === 'RECEIVED') {
                ...
                div.innerHTML = `
                    <button id="regen-btn" class="btn btn-primary" onclick="regenerateDraft(${currentDialogueId}, this)">⚡ Generate Reply</button>
                    <button id="prompt-btn" class="btn btn-secondary" onclick="openPromptModal(${currentDialogueId})">📝 Rules / Prompt</button>
                `;
                container.appendChild(div);
        ...
        switchView = (viewId) => {
            // Hide all views
            document.querySelectorAll('.view-container, #inbox-view, .main-content').forEach(el => {
                el.classList.remove('active');
